...
// Dispute correspondence timeline. Date in secondary on the left, actor + summary
// on a card on the right. "Detected from inbox" badge marks auto-assembled
// correspondence; the settled item gets a full success tint + a green check.
export default function CorrespondenceTimeline({
  ...
  return (
    <section>
      <h2 className="text-label-caps text-secondary">Dispute timeline</h2>
          ...
          <li key={`${item.date}-${i}`} className="flex gap-5">
            ...
            <div
              className={`flex-1 rounded-xl border p-5 ${
                item.settled
                  ? "border-transparent bg-success-container"
