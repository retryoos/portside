...
- **Single source of truth** for the demo voyage = **MT Aegean Pioneer, Ras Tanura → Rotterdam, USD 84,375.00** (72h allowed, 117h used, 45h on demurrage at USD 1,875/hr; contested 4h weather stoppage on 17 May, CP clause 14 precipitation > 0.5 mm/hr, authority *The Mexico 1* [1990] 1 Lloyd's Rep 507). Lives in `apps/web/DESIGN.md` = `apps/web/lib/demo.ts` = `apps/api/portside_api/fixtures.py`. **Do not reintroduce the old Athens/Piraeus/38,400 scenario.**
...
- `apps/api/tests/test_calculator.py` — **the gate.** Lock the arithmetic to the Rotterdam worked example (72 / 117 / 45h → **USD 84,375.00** at 1,875/hr). Do not merge if this fails.
- `synthetic-data/` — generate the Rotterdam demo PDFs (CP, NOR, SoF) + `expected.json`, reconciling to USD 84,375. `weasyprint`, **dev-time only** (never in the runtime/App Runner image).
...
**Acceptance:** upload the synthetic PDFs → `GET /voyages/{id}` progresses to `done` with real `extraction` + `laytime`; `test_calculator.py` passes; quantum on the demo PDFs is USD 84,375.00.
