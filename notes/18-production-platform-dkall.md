...
- **A5 — Edit-with-AI persist (pairs Roman C1).** Endpoint to write an *accepted* revision into the stored `ClaimPacket` markdown, so edits survive reload + reach the PDF. Files: `main.py` route (or `POST /voyages/{id}/revise/apply`), `storage.py` patch. *Accept:* accept → reload → persists → in exported PDF. **Deep spec:** [new_features/01 — Edit with AI](new_features/01-edit-with-ai.md) (you own Phases 2 + 4).
- **A6 — Both sides of the deal (pairs Roman C4).** `POST /voyages/{id}/rebut` → charterer rebuttal (`Perspective="charterer"` + `charterer_argument` already exist); deterministic swing **84,375.00 → 76,875.00** (4h × 1,875). Keep models feature-local in `defense.py` (schemas frozen). **Lock 76,875.00 in `tests/test_defense.py`.** Files: `defense.py`, `prompts/rebuttal.md`, route. *Accept:* test asserts 76,875.00; owner gate untouched. **Deep spec:** [new_features/03 — Both sides](new_features/03-both-sides-defense.md).
- **A7 — Research agents (pairs Roman C3).** Tool-using evidence agent filling the unused `FlaggedEvent.evidence_required`; committed Rotterdam fixture for offline; live API behind a flag. Adds a `researching` pipeline stage — that's a frozen-schema change, **announce first**. Files: `agents/researcher.py`, `agents/tools.py`, `prompts/researcher.md`, fixture. *Accept:* offline run returns sourced evidence per disputed hour. **Deep spec:** [new_features/02 — Research agents](new_features/02-research-agents.md).
